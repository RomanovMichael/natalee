...
$form['callback'] = array(
	'fields' => array(
		...
		'tel' => array(
			...
			// 'validate' => array(
			// 	'required' => true,
			// 	'preg'     => '/\+0-9\s\([0-9]{3}\)\s[0-9]{3}-[0-9]{2}-[0-9]{2}/',
			// ),
			// 'messages' => array(
...
if (isset($form[$act])) {
	...
	if (count($error) == 0) {
		...
		// $queryUrl = 'https://b24-nmqljg.bitrix24.ru/rest/1/14rkjjc6wov5vyl4/crm.lead.add.json';
		$queryData = http_build_query(array(
		'FIELDS' => array(
			'TITLE' => 'Заявка с сайта', // НАЗВАНИЕ
			'NAME' => $_POST['name'], // ИМЯ
			'COMMENTS' => $_POST['massage'] . ' ' . $_POST['services'],
			'PHONE' => Array(
				"n0" => Array(
					"VALUE" => $_POST['tel'],
					"VALUE_TYPE" => "WORK",
			...
			// 'ASSIGNED_BY_ID' => intval($_POST['_assigned']), // Ид ответственного
